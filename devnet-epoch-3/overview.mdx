...
- <span style={{color: "red"}}>Exists only on Solana Devnet with no monetary value</span>
...
<Warning>
  \$INT-DEV tokens are test tokens with no monetary value. All staking and rewards during Epoch 3 are for testing purposes only.
  
   $INT-DEV accumlated during Epoch 3 will not be transferable to mainnet, and will have no impact on your mainnet balance.
</Warning>
