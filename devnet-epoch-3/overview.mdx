...
| Date | Features Going Live | Description |
|------|-------------------|-------------|
...
| **Late June 2025** | **Advanced Features** | • Full reputation scoring activation<br/>• Performance-based routing adjustments<br/>• Slashing mechanism testing (testnet only)<br/>• Token faucet for additional $INT-DEV |
...
- **Minimum Self-Stake**: Currently set at 10% of total pool stake

Operators must maintain the minimum self-stake ratio to ensure alignment between pool performance and operator incentives. If delegations cause the self-stake ratio to fall below 10%, the operator must add additional stake or risk pool deactivation.

### Delegation Mechanics

Token holders can delegate $INT-DEV to operator pools to earn a share of rewards without running hardware. The delegation process:
1. Tokens can be delegated immediately without cooldown
2. Delegators earn rewards proportional to their stake percentage
3. Rewards are calculated after operator commission
4. Undelegating requires a 7-day cooldown period
...
This data will inform the eventual transition to mainnet, where $INT tokens will replace the current testnet implementation.
