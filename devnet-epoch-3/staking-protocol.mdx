...
description: "Understanding how the Inference.net staking protocol works on Solana"
---
...
Operators must maintain at least 10% of their pool's total stake to ensure alignment with delegators.
...
3. **No Cooldown**: Staking has no waiting period
...
2. **Cooldown Period**:
   - Operators have longer cooldowns for stability
   - Delegators have shorter cooldowns for flexibility
...
- **Halt Authorities**: Emergency response capabilities
- **Slashing Authorities**: Enforce performance standards

## Economic Alignment

The protocol creates several layers of economic alignment:

1. **Minimum Operator Stake**: Ensures operators have skin in the game
2. **Stake-Weighted Routing**: Higher stake increases job allocation probability
...
- Operators create pools with initial configuration
- Automatic staking record creation for the operator
- Token accounts initialized for stake and fee management

### Operation

- Accept delegations (if enabled)
- Process inference jobs based on stake weight
- Accumulate rewards each epoch
- Maintain minimum stake requirements

### Closure

- Operators can close pools to new stakes
- Existing stakes can still be withdrawn
- Rewards stop accumulating after closure
- Pool remains on-chain for historical record
